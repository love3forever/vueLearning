...
def get_typhoon_info(typhoonid):
	...
	for item in typhoon_detail:
	    ...
	    typhoon_data['topspeed'] = '{}m/s'.format(item[7]) if item[7] !=0 else 0
	    typhoon_data['pressure'] = '{}百帕'.format(item[6]) if item[6] !=0 else 0
	    typhoon_data['movespeed'] = '{}公里/小时'.format(item[9]) if item[9] !=0 else 0
